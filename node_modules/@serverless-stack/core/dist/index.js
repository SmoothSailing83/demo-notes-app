...
function runCdkSynth(cdkOptions) {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        child.stderr.on("data", (data) => {
            ...
            if (dataStr.startsWith("Subprocess exited with error 1") ||
...
async function deployStack(cdkOptions, stackState) {
    ...
    // CF update NOT started + Deploy failed
        ...
        if (stdOutput.indexOf("This stack uses assets, so the toolkit stack must be deployed") > -1) {
            statusReason = "not_bootstrapped";
        }
...
async function destroyInit(cdkOptions, stackName) {
    ...
    stacks.forEach(({ name, dependencies }) => dependencies.forEach((dep) => {
...
async function destroyPoll(cdkOptions, stackStates) {
    const destroyStacks = async () => {
        ...
        await Promise.all(stackStates
            ...
            .filter((stackState) => stackState.dependencies.every((dep) => statusesByStackName[dep] === STACK_DESTROY_STATUS.SUCCEEDED))
            .map(async (stackState) => {
            try {
...
module.exports = {
    ...
    Packager: packager_1.Packager,
    State: state_1.State,
    Runtime: runtime_1.Runtime,
    Bridge: bridge_1.Bridge,
};
